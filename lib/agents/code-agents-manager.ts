 ...
 */
export async function executeMultiAgents(request: MultiAgentRequest): Promise<MultiAgentResponse> {
  const startTime = Date.now()
...
function generateSummary(results: AgentResult[], duration: number): string {
  const agentNames = results.map((r) => AGENT_CAPABILITIES[r.agentType].name)
  const successCount = results.filter((r) => r.status === "completed").length
  const errorCount = results.filter((r) => r.status === "error").length
